        ...
        if (currentState is CatsLoaded) {
          ...
          yield cats.isEmpty
              ...
              : currentState.favs.isEmpty
                  ? CatsLoaded(
                      favs: await _getFavs(event.email),
    ...
    if (event is ToggleFavorite) {
      try {
        if (currentState is CatsLoaded) {
          ...
          final favs = await _getFavs(event.email);
          yield favs.isEmpty
              ? CatsLoaded(favs: [], cats: currentState.cats, hasReachedMax: false)
              : CatsLoaded(
                  favs: favs, hasReachedMax: false, cats: currentState.cats);
        }
      } catch (_) {
        print("error toggle");
      }
...
//
  ...
  Future<List<Map<String, String>>> _fetchImageUrls(int limit) async {
    final imageUrl =
        'https://api.thecatapi.com/v1/images/search?mime_types=jpg,png&limit=$limit';
